  ...
  quench.registerBatch(
    ...
    (context) => {
      ...
      after(async function () {
        ...
        for (const note of allNotes) if (note.name?.startsWith(TEST_NOTE_PREFIX)) await api.deleteNote(note.id);
      });
      ...
      describe('Note Properties', function () {
        ...
        it('updateNote changes note content and returns updated note', async function () {
          ...
          const updated = await api.updateNote(createdPageId, { name: `${TEST_NOTE_PREFIX} Updated Properties` });
        ...
        it('note with preset preserves categories field', function () {
          ...
          assert.include(note.flagData.categories, 'event', 'Should contain the event preset');
        });
