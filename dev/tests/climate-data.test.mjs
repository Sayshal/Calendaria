...
describe('CLIMATE_ZONE_TEMPLATES', () => {
  ...
  it('has all expected templates', () => {
    for (const id of templateIds) expect(CLIMATE_ZONE_TEMPLATES, `Missing template: ${id}`).toHaveProperty(id);
  });
  ...
  it('all weather entries reference valid preset IDs', () => {
    ...
    for (const [templateId, template] of Object.entries(CLIMATE_ZONE_TEMPLATES)) {
      for (const [season, presets] of Object.entries(template.weather)) {
        for (const presetId of Object.keys(presets)) expect(validIds.has(presetId), `${templateId}.weather.${season} references unknown preset "${presetId}"`).toBe(true);
      }
    }
  });
  it('all templates have valid windSpeedRange', () => {
    for (const [id, template] of Object.entries(CLIMATE_ZONE_TEMPLATES)) expect(template.windSpeedRange.min, `${id} windSpeedRange.min`).toBeLessThanOrEqual(template.windSpeedRange.max);
  });
});
