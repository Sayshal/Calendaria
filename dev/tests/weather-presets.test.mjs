...
describe('Weather Preset Data Integrity', () => {
  ...
  it('all presets have valid category values', () => {
    ...
    for (const preset of ALL_PRESETS) expect(validCategories, `${preset.id} has invalid category "${preset.category}"`).toContain(preset.category);
  });
  it('all presets have valid hex colors', () => {
    const hexRegex = /^#[\dA-Fa-f]{6}$/;
    for (const preset of ALL_PRESETS) expect(hexRegex.test(preset.color), `${preset.id} has invalid color "${preset.color}"`).toBe(true);
  });
  it('all preset hudEffects are in HUD_EFFECTS list', () => {
    for (const preset of ALL_PRESETS) {
      if (preset.hudEffect) expect(HUD_EFFECTS, `${preset.id} has unknown hudEffect "${preset.hudEffect}"`).toContain(preset.hudEffect);
    }
  });
  it('all preset soundFx values are valid or null', () => {
    for (const preset of ALL_PRESETS) {
      if (preset.soundFx) expect(SOUND_FX_OPTIONS, `${preset.id} has unknown soundFx "${preset.soundFx}"`).toContain(preset.soundFx);
    }
  });
...
describe('getPresetAlias()', () => {
  ...
  it('returns alias when scoped to calendar and zone', () => {
    game.settings.get.mockReturnValue({ gregorian: { temperate: { clear: 'Sunny' } } });
...
describe('SOUND_FX_OPTIONS full paths', () => {
  it('all entries are full Foundry-resolvable paths', () => {
    for (const path of SOUND_FX_OPTIONS) {
      expect(path).toMatch(/^modules\/calendaria\/assets\/sound\/.+\.ogg$/);
    }
  });
  it('all preset soundFx values are full paths or null', () => {
    for (const preset of ALL_PRESETS) {
      if (preset.soundFx) {
        expect(preset.soundFx, `${preset.id} soundFx is not a full path`).toContain('/');
        expect(preset.soundFx).toMatch(/\.ogg$/);
      }
    }
  });
});
