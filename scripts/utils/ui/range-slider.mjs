...
export class RangeSlider {
  ...
  #calculateBoundaries(targetPercent, handleIdx) {
    ...
    for (let i = handleIdx; i < this.positions.length - 1; i++) if (this.positions[i + 1] < this.positions[i] + this.minGap) this.positions[i + 1] = this.positions[i] + this.minGap;
    for (let i = handleIdx; i > 0; i--) if (this.positions[i - 1] > this.positions[i] - this.minGap) this.positions[i - 1] = this.positions[i] - this.minGap;
  ...
  #onDoubleClick(segmentIdx) {
    ...
    for (let i = 0; i < this.positions.length; i++) this.positions[i] = boundaries[i].end;
    this.#updateVisuals(boundaries);
    this.#updateInputs(boundaries);
    this.#commitData(boundaries);
  }
  ...
  destroy() {
    const track = this.#getTrack();
    if (!track) return;
    for (const handle of track.querySelectorAll(this.handleSelector)) delete handle.dataset.rangeSliderAttached;
    for (const segment of track.querySelectorAll(this.segmentSelector)) delete segment.dataset.rangeSliderAttached;
    for (let i = 0; i < this.numSegments; i++) {
      const key = this.keys[i];
      const startInput = this.container.querySelector(this.inputStartSelector.replace('{key}', key));
      const endInput = this.container.querySelector(this.inputEndSelector.replace('{key}', key));
