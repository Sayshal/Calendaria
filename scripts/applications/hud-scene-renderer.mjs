 ...
 * Renders sky gradient, stars, weather particles, sun, moon, and flash effects
 * in a single PIXI application per view (dome/slice).
...
/**
 * Linearly interpolate between two hex color strings. Returns [r,g,b].
 * @param {string} color1 - Start hex color string
 * @param {string} color2 - End hex color string
...
/**
 * Convert [r,g,b] array to PIXI hex number.
 ...
 * Manages 6 layers: sky, stars, weather particles, sun, moon, flash overlay.
 */
export class HudSceneRenderer {
  ...
  constructor(canvas, mode = 'dome') {
    ...
    this.#buildSunLayer();
    this.#buildMoonLayer();
    // Flash overlay created on-demand
  ...
  #buildStarLayer() {
    ...
    // Shared 2x2 white circle texture
    ...
    for (let i = 0; i < STAR_COUNT; i++) {
      ...
      sprite.x = rand(4, w - 4);
      sprite.y = rand(4, h * 0.85); // Stars mostly above horizon
  ...
  #buildSunLayer() {
    // Generate sun texture: 32x32 radial gradient via off-screen canvas
    const size = 32;
    ...
    // Corona: same texture at 2x scale, low alpha
  ...
  /** Build the moon layer (Layer 4). Creates container and shared texture only. */
  #buildMoonLayer() {
    const size = 24;
    const offCanvas = document.createElement('canvas');
    offCanvas.width = size;
   ...
   * First call sets bg immediately; subsequent calls trigger a crossfade.
  ...
  #updateSky(colors) {
    ...
    // First call — set bg immediately, no fade
    ...
    // Subsequent calls — put new texture on fg, start crossfade
   ...
   * @param {object} c - Sky color set with top, mid, and bottom RGB arrays
  ...
  #updateMoons(hour, sunrise, sunset, hoursPerDay, moons) {
    ...
    // Gradual fade: 0.25 hours (~15 min) fade near sunrise/sunset
    const fadeDuration = 0.25;
    let moonAlpha = 1;
    if (hour >= sunrise && hour < sunset) {
      // Daytime — check if we're in the fade-out window just after sunrise
      if (hour < sunrise + fadeDuration) {
        moonAlpha = 1 - (hour - sunrise) / fadeDuration;
      } else {
        moonAlpha = 0;
      }
    } else {
      // Nighttime — check if we're in the fade-in window just after sunset
      if (hour >= sunset && hour < sunset + fadeDuration) {
        moonAlpha = (hour - sunset) / fadeDuration;
      }
      // Also check fade-out approaching sunrise
      if (hour < sunrise && hour > sunrise - fadeDuration) {
        moonAlpha = (sunrise - hour) / fadeDuration;
      }
    }
    ...
    // Trailing spacing: each secondary moon is offset backwards along the arc
    // Use moon-diameter-based spacing so they don't overlap
    ...
    // Convert pixel spacing to hours along the arc
    let arcPixels;
    if (this.#mode === 'dome') {
      const trackWidth = w > 120 ? 140 : 100;
      const trackHeight = w > 120 ? 70 : 50;
      const arcRadius = Math.min(trackWidth / 2, trackHeight) - baseMoonSize / 2 - 4;
      arcPixels = arcRadius * Math.PI; // half-circle arc length in pixels
    } else {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Trail behind primary: each moon is offset backwards along the arc
      ...
      // ── Phase mask: mask sprite + glow to lit portion only ──
      shadow.clear();

      // New moon — hide entirely
      ...
      // Full moon — show entire sprite, no mask needed
      ...
      // Draw lit-area shape using proper terminator ellipse
      // Terminator rx: r at new → 0 at quarter → -r at full
      const terminatorRx = rad * Math.cos(illumination * Math.PI);
      const k = 0.5522847498; // bezier kappa for circle approximation

      shadow.beginFill(0xffffff);
      if (isWaxing) {
        // Lit on right: right semicircle arc + terminator curve back
        shadow.moveTo(moonX, moonY - rad);
        shadow.arc(moonX, moonY, rad, -Math.PI / 2, Math.PI / 2);
        // Terminator: from (moonX, moonY+rad) back to (moonX, moonY-rad)
        // via (moonX + terminatorRx, moonY)
        ...
        // Lit on left: left semicircle arc + terminator curve back
        shadow.moveTo(moonX, moonY - rad);
      ...
      // Apply as mask to both sprite and glow
      sprite.mask = shadow;
      glow.mask = shadow;

      // Clean up previous separate mask if any
      if (shadow._prevMask && shadow._prevMask !== shadow) {
        shadow._prevMask.destroy();
      }
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // Weather particles (absorbed from HudWeatherRenderer)
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  /** Build the particle pool for the current effect. */
  #buildParticles() {
  ...
  #tick(delta) {
    ...
    // Sky crossfade — lerp fg alpha toward 1, then promote fg to bg
    if (this.#skyFading) {
      this.#skyFade = Math.min(1, this.#skyFade + 0.02 * delta);
      this.#skySpriteFg.alpha = this.#skyFade;
      if (this.#skyFade >= 1) {
        // Promote: move fg texture to bg, clear fg
