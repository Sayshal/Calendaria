  ...
  static patchBaseSceneConfig() {
    ...
    Base.prototype._preparePartContext = async function (partId, context, options) {
      ...
      if (partId === 'calendaria') {
        ...
        context.fxBackgroundOverride = doc.getFlag(MODULE.ID, SCENE_FLAGS.FXMASTER_BACKGROUND_OVERRIDE) ?? 'default';
        context.fxTokenTrailsOverride = doc.getFlag(MODULE.ID, SCENE_FLAGS.FXMASTER_TOKEN_TRAILS_OVERRIDE) ?? 'default';
        context.fxAboveDarknessOverride = doc.getFlag(MODULE.ID, SCENE_FLAGS.FXMASTER_ABOVE_DARKNESS_OVERRIDE) ?? 'default';
